...
var healthport = session.parameters.healthport
  ? session.parameters.healthport
  : 0;
var evaluator = session.parameters.evaluator;

console.debug(
  "in custom healthcheck.js for server %s:%d from lb-group %s",
  svr,
  healthport,
  grp
);
...
if (headers.current.statusCode != 200) {
  session.reject(
    sf(
      "Server %s:%d from lb-group %s is unhealthy, status code: %d",
      svr,
      healthport,
      grp,
      headers.current.statusCode
    )
  );
} else {
  ...
  session.input.readAsJSON(function (readAsJSONError, jsonResponse) {
    ...
    if (readAsJSONError) {
      session.reject(
        sf(
          "Server %s:%d from lb-group %s did not return at JSON response, readAsJSON error code: %d",
          svr,
          healthport,
          grp,
          readAsJSONError.errorCode
        )
      );
    } else {
      ...
      try {
        ...
        for (var property in evaluatorObject) {
          if (evaluatorObject[property] != jsonResponse[property]) {
            session.reject(
              sf(
                "Server %s:%d from lb-group %s healthcheck response invalid, expected: %s received %s",
                svr,
                healthport,
                grp,
                evaluatorObject[property],
                jsonResponse[property]
              )
            );
          } else {
            console.debug(
              "in custom healthcheck.js for server %s:%d from lb-group %s, successful evaluation of property %s",
              svr,
              healthport,
              grp,
              property
        ...
        session.reject(
          sf(
            "Server %s:%d from lb-group %s evaluator is not a JSON object, errorMessage: %s",
            svr,
            healthport,
            grp,
            err.errorMessage
